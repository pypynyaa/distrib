...
function normalizeSmartLink(item){
  ...
  return {...item,url,links:labels,rawLinks,active:item.active!==false&&item.active!==0,clicks:item.clicks||0};
}
...
function PromoLinks({links,onCreateLink,onUpdateLink,onDeleteLink,releases=[]}){
 const firstRelease=releases[0]||{id:'',title:'Новый релиз'};
 const emptyForm={release_id:firstRelease.id,title:firstRelease.title,slug:makeSlug(firstRelease.title),yandex:'https://music.yandex.ru',vk:'https://vk.com/music',apple:'https://music.apple.com',spotify:'',active:true};
 ...
 const submit=()=>{const bad=['yandex','vk','apple','spotify'].find(key=>form[key]&& !isValidUrl(form[key]));if(bad){setError('Проверьте ссылки: каждая должна начинаться с https://');return}setError('');editing?onUpdateLink(editing,form):onCreateLink(form);setEditing(null);setForm(emptyForm)};
 const edit=item=>{const raw=item.rawLinks||{};setEditing(item.id);setForm({release_id:item.release_id||'',title:item.title||'',slug:item.slug||makeSlug(item.title),yandex:raw['Яндекс Музыка']||'',vk:raw['VK Музыка']||'',apple:raw['Apple Music']||'',spotify:raw.Spotify||'',active:item.active!==false});window.scrollTo({top:0,behavior:'smooth'})};
 const toggle=item=>onUpdateLink(item.id,{release_id:item.release_id,title:item.title,slug:item.slug,yandex:item.rawLinks?.['Яндекс Музыка']||'',vk:item.rawLinks?.['VK Музыка']||'',apple:item.rawLinks?.['Apple Music']||'',spotify:item.rawLinks?.Spotify||'',active:item.active===false});
 ...
 return <div className="page fade promo-links-page"><div className="page-title"><div><span className="eyebrow">SMART LINKS</span><h1>Промо-ссылки</h1><p>Создавайте одну красивую страницу, где слушатель увидит все площадки релиза.</p></div><button className="primary" onClick={submit}>{editing?<Check/>:<Plus/>}{editing?'Сохранить линкс':'Создать линкс'}</button></div><div className="panel link-builder smart-form"><PanelHead title={editing?'Редактирование линкса':'Новый линкс'} action={editing?'Отменить':''} onClick={()=>{setEditing(null);setForm(emptyForm)}}/>{error&&<div className="auth-error">{error}</div>}<div className="field-row"><label>Релиз<select value={form.release_id} onChange={e=>updateRelease(e.target.value)}>{releaseOptions.map(r=><option key={r.id} value={r.id}>{r.title}</option>)}</select></label><label>Slug<input value={form.slug} onChange={e=>set('slug',makeSlug(e.target.value))} placeholder="midnight-echoes"/><small className="field-hint">Будет ссылка: {publicLinkUrl(form.slug)}</small></label></div><label>Название линкса<input value={form.title} onChange={e=>set('title',e.target.value)} placeholder="Название релиза или кампании"/></label><div className="field-row"><label>Яндекс Музыка<input value={form.yandex} onChange={e=>set('yandex',e.target.value)} placeholder="https://music.yandex.ru/..."/></label><label>VK Музыка<input value={form.vk} onChange={e=>set('vk',e.target.value)} placeholder="https://vk.com/..."/></label></div><div className="field-row"><label>Apple Music<input value={form.apple} onChange={e=>set('apple',e.target.value)} placeholder="https://music.apple.com/..."/></label><label>Spotify<input value={form.spotify} onChange={e=>set('spotify',e.target.value)} placeholder="необязательно"/></label></div><label className="toggle-line"><input type="checkbox" checked={form.active} onChange={e=>set('active',e.target.checked)}/><span>Линкс активен и доступен публично</span></label></div>{links.length===0&&<div className="panel empty-state"><Link/><h3>Промо-ссылок пока нет</h3><p>Выберите релиз, добавьте ссылки на площадки и нажмите «Создать линкс».</p></div>}<div className="link-grid">{links.map(item=><article className={`link-card ${item.active===false?'disabled-link':''}`} key={item.id}><div><Link/><span>{item.title}</span></div><div className="link-meta"><small>{item.active===false?'Выключен':'Активен'}</small><small>{Number(item.clicks||0).toLocaleString('ru')} переходов</small></div><a className="link-url" href={item.url} target="_blank" rel="noreferrer">{item.url}</a><div className="platform-chips">{(item.links||[]).map(x=><i key={x}>{x}</i>)}</div><div className="link-actions"><a className="secondary" href={item.url} target="_blank" rel="noreferrer"><ArrowRight/>Открыть</a><button className="secondary" onClick={()=>copy(item.url)}><Copy/>{copied===item.url?'Скопировано':'Скопировать'}</button><button className="secondary" onClick={()=>edit(item)}><Edit3/>Редактировать</button><button className="secondary" onClick={()=>toggle(item)}>{item.active===false?<Check/>:<X/>}{item.active===false?'Включить':'Выключить'}</button><button className="secondary danger-btn" onClick={()=>onDeleteLink(item.id)}><X/>Удалить</button></div></article>)}</div></div>
}
...
function AdminAccounts({users,onBlockUser}){
 ...
 return <div className="panel admin-table accounts-table"><div className="tr th"><span>Кабинет</span><span>Почта</span><span>Регистрация</span><span>Статус</span></div>{users.map(u=><div className="tr" key={u.id}><span className="admin-name"><i>{u.artist.split(' ').map(x=>x[0]).join('').slice(0,2)}</i><b>{u.artist}</b>{u.reason&&<small>Причина: {u.reason}</small>}</span><span>{u.email}</span><span>{u.registered}</span><span className={u.status==='Заблокирован'?'danger':'online'}>{u.status}{u.status!=='Заблокирован'&&<input value={reasons[u.id]||''} onChange={e=>setReasons(r=>({...r,[u.id]:e.target.value}))} placeholder="Причина блокировки"/>}<button onClick={()=>onBlockUser(u.id,reasons[u.id])}><Ban/>{u.status==='Заблокирован'?'Разблокировать':'Блок'}</button></span></div>)}</div>
}
...
function App(){
 ...
 useEffect(()=>{if(!logged)return;const t=token();if(t==='demo')return;api('/news').then(setNews).catch(()=>{});api('/notifications',{token:t}).then(rows=>setNotifications(rows.map(x=>({...x,read:!!x.read})))).catch(()=>{});api('/releases',{token:t}).then(rows=>setReleases(rows.map(normalizeRelease))).catch(e=>pushToast('Не удалось загрузить релизы',e.message,'error'));api('/finance',{token:t}).then(fin=>{setFinance({balance:fin.balance||0,accrued:fin.accrued||0,paid:fin.paid||0,pending:fin.pending||0,history:fin.history||[]});setPayouts((fin.payouts||[]).map(p=>({id:p.id,userId:p.user_id,artist:currentUser.artist,amount:p.amount,card:p.card_number,holder:p.holder,bank:p.bank,reason:p.rejection_reason,status:p.status,date:p.created_at})))}).catch(()=>{});api('/promo/requests',{token:t}).then(rows=>setPromoRequests(rows)).catch(()=>{});if(currentUser.role!=='artist'){api('/admin/accounts',{token:t}).then(rows=>setUsers(rows.map(u=>({id:u.id,email:u.email,artist:u.artist_name,role:u.role,status:u.active?'Активен':'Заблокирован',reason:u.block_reason,registered:u.created_at,balance:u.balance||0,paid:u.paid||0})))).catch(()=>{});api('/admin/payouts',{token:t}).then(rows=>setPayouts(rows.map(p=>({id:p.id,userId:p.user_id,artist:p.artist_name,amount:p.amount,card:p.card_number,holder:p.holder,bank:p.bank,reason:p.rejection_reason,status:p.status,date:p.created_at})))).catch(()=>{});api('/admin/users',{token:t}).then(rows=>setAdmins(rows.map(u=>({id:u.id,initials:u.artist_name.split(' ').map(x=>x[0]).join('').slice(0,2),name:u.artist_name,role:u.role,access:'Админка',active:u.active})))).catch(()=>{})}else{api('/smart-links',{token:t}).then(rows=>setPromoLinks(rows.map(normalizeSmartLink))).catch(e=>pushToast('Не удалось загрузить промо-ссылки',e.message,'error'))}},[logged,currentUser.role]);
 ...
 const requestPayout=async data=>{try{const amount=Number(data.amount);const card=data.card;const t=token();let item={id:Date.now(),userId:currentUser.id,artist:currentUser.artist,amount,card,status:'Ожидает выплаты',date:'сегодня',holder:data.holder,bank:data.bank};if(t!=='demo'){const saved=await api('/finance/payouts',{method:'POST',token:t,body:{amount,card_number:card,holder:data.holder,bank:data.bank}});item={...item,id:saved.id}}setPayouts(list=>[item,...list]);setFinance(f=>({...f,balance:Math.max((f.balance||0)-amount,0),pending:(f.pending||0)+amount}));pushToast('Заявка отправлена','Команда увидит её в админке');pushNotification('Заявка на выплату',`Запрошено ${amount.toLocaleString('ru')} ₽`) }catch(e){pushToast('Ошибка выплаты',e.message,'error')}};
 const updatePayoutStatus=async(id,status,reason='')=>{try{const t=token();if(t!=='demo')await api(`/admin/payouts/${id}`,{method:'PATCH',token:t,body:{status,reason}});const payout=payouts.find(p=>p.id===id);setPayouts(list=>list.map(p=>p.id===id?{...p,status,reason:status==='Отклонено'?reason:''}:p));if(status==='Оплачено'&&payout){setUsers(list=>list.map(u=>u.id===payout.userId?{...u,paid:(u.paid||0)+Number(payout.amount||0)}:u))}pushToast(status==='Оплачено'?'Выплата оплачена':'Выплата отклонена','Статус заявки обновлён')}catch(e){pushToast('Ошибка выплаты',e.message,'error')}};
 const blockUser=async(id,reason='')=>{try{const user=users.find(u=>u.id===id);const active=user?.status==='Заблокирован';const finalReason=active?'':(reason||'Нарушение правил сервиса');const t=token();if(t!=='demo')await api(`/admin/accounts/${id}`,{method:'PATCH',token:t,body:{active,reason:finalReason}});setUsers(list=>list.map(u=>u.id===id?{...u,status:u.status==='Заблокирован'?'Активен':'Заблокирован',reason:active?'':finalReason}:u));pushToast(active?'Кабинет разблокирован':'Кабинет заблокирован',user?.artist||'Пользователь')}catch(e){pushToast('Ошибка кабинета',e.message,'error')}};
 ...
 const createPromoLink=async data=>{try{const title=typeof data==='string'?data:data.title;const raw=typeof data==='string'?{slug:makeSlug(data),yandex:'https://music.yandex.ru',vk:'https://vk.com/music',apple:'https://music.apple.com',active:true}:data;const links={};[['Яндекс Музыка','yandex'],['VK Музыка','vk'],['Apple Music','apple'],['Spotify','spotify']].forEach(([name,key])=>{if(raw[key])links[name]=raw[key]});if(!title?.trim())throw new Error('Выберите релиз или укажите название линкса');if(!Object.keys(links).length)throw new Error('Добавьте хотя бы одну ссылку на площадку');Object.entries(links).forEach(([name,url])=>{if(!isValidUrl(url))throw new Error(`${name}: ссылка должна начинаться с https://`)});const t=token();let slug=makeSlug(raw.slug||title);let item=normalizeSmartLink({id:Date.now(),title,slug,url:publicLinkUrl(slug),links,active:raw.active!==false,clicks:0});if(t!=='demo'){const saved=await api('/smart-links',{method:'POST',token:t,body:{release_id:Number(raw.release_id)||null,title,slug,links,active:raw.active!==false}});item=normalizeSmartLink(saved)}setPromoLinks(list=>[item,...list]);pushToast('Промо-ссылка создана',item.url)}catch(e){pushToast('Ошибка линкса',e.message,'error')}};
 const updatePromoLink=async(id,data)=>{try{const title=data.title;const links={};[['Яндекс Музыка','yandex'],['VK Музыка','vk'],['Apple Music','apple'],['Spotify','spotify']].forEach(([name,key])=>{if(data[key])links[name]=data[key]});if(!title?.trim())throw new Error('Укажите название линкса');if(!Object.keys(links).length)throw new Error('Добавьте хотя бы одну ссылку на площадку');Object.entries(links).forEach(([name,url])=>{if(!isValidUrl(url))throw new Error(`${name}: ссылка должна начинаться с https://`)});const t=token();let item=normalizeSmartLink({id,title,slug:makeSlug(data.slug||title),url:publicLinkUrl(makeSlug(data.slug||title)),links,active:data.active!==false,release_id:Number(data.release_id)||null,clicks:promoLinks.find(x=>x.id===id)?.clicks||0});if(t!=='demo'){const saved=await api(`/smart-links/${id}`,{method:'PATCH',token:t,body:{release_id:Number(data.release_id)||null,title,slug:makeSlug(data.slug||title),links,active:data.active!==false}});item=normalizeSmartLink(saved)}setPromoLinks(list=>list.map(x=>x.id===id?item:x));pushToast('Промо-ссылка обновлена',item.url)}catch(e){pushToast('Ошибка линкса',e.message,'error')}};
