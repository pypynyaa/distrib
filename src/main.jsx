...
function SearchBox({items,setPage}){
  ...
  return <div className="search-wrap"><button className={`search ${open?'active':''}`} onClick={()=>setOpen(true)}><Search/>{open?<input autoFocus value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Escape'&&setOpen(false)} placeholder="Поиск по кабинету..."/>:'Поиск'}</button>{open&&<div className="dropdown search-results"><div className="drop-head"><b>Поиск</b><button onClick={()=>setOpen(false)}><X/></button></div>{!query&&<p>Введите название релиза, раздела, новости или промо-ссылки.</p>}{query&&results.length===0&&<p>Ничего не найдено</p>}{results.map(item=><button key={`${item.type}-${item.id}`} onClick={()=>select(item)}><span>{item.type}</span><b>{item.title}</b><small>{item.subtitle}</small></button>)}</div>}</div>
}

function NotificationMenu({items,onReadAll}){
  const [open,setOpen]=useState(false);
  const unread=items.filter(x=>!x.read).length;
  return <div className="menu-wrap"><button className="icon notification" onClick={()=>setOpen(!open)}><Bell/>{unread>0&&<i/>}</button>{open&&<div className="dropdown notify-menu"><div className="drop-head"><b>Уведомления</b><button onClick={onReadAll}>Прочитано</button></div>{items.length===0&&<p>Пока нет уведомлений.</p>}{items.slice(0,8).map(item=><article className={item.read?'read':''} key={item.id}><span>{item.channel||'system'}</span><b>{item.subject}</b><p>{item.body}</p></article>)}</div>}</div>
}
...
function ReleaseDetail({release,setPage,onPromo,onLink}){
 ...
 return <div className="page fade release-detail"><button className="back" onClick={()=>setPage('Релизы')}>← Назад к релизам</button><div className="detail-hero panel"><div>{coverUrl?<img className="detail-cover-img" src={coverUrl} alt={release.title}/>:<Cover kind={release.cover} large/>}</div><div className="detail-info"><span className="eyebrow">КАРТОЧКА РЕЛИЗА</span><h1>{release.title}</h1><p>{release.artist} · {release.type} · {release.genre}</p><Badge status={release.status}/>{release.reason&&<div className="reject"><CircleHelp/><div><b>Причина отклонения</b><p>{release.reason}</p></div></div>}<div className="detail-actions"><button className="primary" onClick={()=>onPromo(release)}><Megaphone/>Подать на промо</button><button className="secondary" onClick={()=>onLink(release)}><Link/>Создать линкс</button></div></div></div><div className="grid-two"><div className="panel"><PanelHead title="Данные релиза" action=""/><div className="detail-list"><span>Дата релиза <b>{release.releaseDate||release.date}</b></span><span>Прослушивания <b>{release.streams.toLocaleString('ru')}</b></span><span>UPC <b>{release.upc||'Ожидает присвоения'}</b></span><span>ISRC <b>{release.isrc||'Ожидает присвоения'}</b></span><span>Площадки <b>{release.platforms?.join(', ')||'Не выбраны'}</b></span></div></div><div className="panel"><PanelHead title="Аудио" action=""/>{audioUrl?<audio controls src={audioUrl} className="audio-player"/>:<div className="empty-state small"><FileAudio/><p>Аудиофайл не прикреплён</p></div>}</div></div><div className="panel"><PanelHead title="Комментарий и участники" action=""/><p className="detail-text">{release.comment||'Комментарий не указан.'}</p></div></div>
}

function UploadRelease({setPage,onCreated,onUpload,currentUser}){
 const [step,setStep]=useState(1); const [busy,setBusy]=useState(false);
 const [error,setError]=useState('');
 const [form,setForm]=useState({title:'',release_type:'Сингл',genre:'Pop',release_date:'2026-07-10',language:'Русский',cover_url:'',audio_url:'',main_artist:currentUser.artist||'',music_author:'',lyrics_author:'',lyrics:'',platforms:['Яндекс Музыка','VK Музыка','Звук','Apple Music','Spotify','YouTube Music'],comment:''});
 ...
 const send=async()=>{const message=validate();if(message){setError(message);return}setBusy(true);try{await onCreated({id:Date.now(),title:form.title,artist:currentUser.artist,type:form.release_type,date:new Date().toLocaleDateString('ru'),status:'На модерации',streams:0,cover:'neon',...form,contributors:{main_artist:form.main_artist,music_author:form.music_author,lyrics_author:form.lyrics_author}});setPage('Релизы')}finally{setBusy(false)}}
...
function Finance({payouts,onPayoutRequest,currentUser}){
 const available=86420.5;
...
function News({isStaff=false}){
 ...
 const [items,setItems]=useState(initial); const [selected,setSelected]=useState(null); const [form,setForm]=useState({category:'ГАЙД',title:'',body:''});
 const add=()=>{if(form.title&&form.body){setItems([[form.category,form.title,form.body,'violet'],...items]);setForm({category:'ГАЙД',title:'',body:''})}};
 if(selected)return <div className="page fade upload-page"><button className="back" onClick={()=>setSelected(null)}>← Назад к новостям</button><article className={`news-card ${selected[3]} news-detail-card`}><div><span>{selected[0]}</span><i>IM</i></div><h3>{selected[1]}</h3><p>{selected[2]}</p></article></div>;
 return <div className="page fade"><div className="page-title"><div><span className="eyebrow">INSOMNIA EDITORIAL</span><h1>Новости</h1><p>Гайды, обновления площадок и истории наших артистов.</p></div></div>{isStaff&&<div className="panel admin-form"><PanelHead title="Добавить новость" action=""/><div className="field-row"><label>Категория<input value={form.category} onChange={e=>setForm({...form,category:e.target.value})}/></label><label>Заголовок<input value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/></label></div><label>Текст новости<textarea value={form.body} onChange={e=>setForm({...form,body:e.target.value})}/></label><button className="primary" onClick={add}><Plus/>Опубликовать</button></div>}<div className="news-grid">{items.map((n,i)=><article className={'news-card '+n[3]} key={n[1]}><div><span>{n[0]}</span><i>0{i+1}</i></div><h3>{n[1]}</h3><p>{n[2]}</p><button onClick={()=>setSelected(n)}>Читать<ArrowRight/></button></article>)}</div></div>
}

function Promo({releases,onPromoRequest}){const [form,setForm]=useState({release_id:releases[0]?.id||'',release_info:'',focus_track:'',artist_info:''});const [sent,setSent]=useState(false);const set=(k,v)=>setForm(f=>({...f,[k]:v}));const submit=async()=>{await onPromoRequest({...form,release_id:Number(form.release_id)||null});setSent(true);setForm(f=>({...f,release_info:'',focus_track:'',artist_info:''}))};return <div className="page fade"><section className="promo-hero"><div><span className="eyebrow light"><Sparkles/> ПРОМО-ПОДДЕРЖКА</span><h1>Питчинг релизов<br/><em>для площадок.</em></h1><p>Наша команда отправляет треки редакторам площадок для попадания в плейлисты и на витрины сервисов. Чтобы попасть в рассылку, загрузите релиз минимум за 14 дней до даты выпуска и подготовьте описание.</p><button className="white-btn" onClick={()=>set('release_info','Коротко: о чём релиз, настроение, история создания, похожие артисты, почему трек может зайти редакторам.')}>Как написать пресс-релиз?<ArrowRight/></button></div><div className="promo-rings"><Megaphone/><i/><i/><i/></div></section><div className="panel pitch-form"><PanelHead title="Заявка на питчинг" action=""/>{sent&&<div className="auth-error success">Заявка отправлена команде Insomnia</div>}<label>Выберите релиз<select value={form.release_id} onChange={e=>set('release_id',e.target.value)}>{releases.map(r=><option value={r.id} key={r.id}>{r.title}</option>)}</select></label><label>Информация о релизе<textarea value={form.release_info} onChange={e=>set('release_info',e.target.value)} placeholder="О чём трек, настроение, история создания, референсы"/></label><label>Фокус-трек для EP и альбомов<input value={form.focus_track} onChange={e=>set('focus_track',e.target.value)} placeholder="Название трека"/></label><label>Информация об артисте<textarea value={form.artist_info} onChange={e=>set('artist_info',e.target.value)} placeholder="Кто артист, достижения, ссылки на соцсети"/></label><div className="form-actions"><button className="secondary" onClick={()=>setForm({release_id:releases[0]?.id||'',release_info:'',focus_track:'',artist_info:''})}>Отмена</button><button className="primary" onClick={submit}>Отправить</button></div></div><div className="promo-options">{[['01','Питчинг','Отправим релиз редакторам площадок и поможем оформить заявку.'],['02','Посевы','Нативные размещения в тематических сообществах.'],['03','Стратегия','Соберём индивидуальный план продвижения под бюджет.']].map(x=><article key={x[0]}><span>{x[0]}</span><h3>{x[1]}</h3><p>{x[2]}</p><button onClick={()=>document.querySelector('.pitch-form')?.scrollIntoView({behavior:'smooth'})}>Обсудить<ArrowRight/></button></article>)}</div></div>}

function PromoLinks({links,onCreateLink,onDeleteLink,releases=[]}){
 const first=releases[0]?.title||'Новый релиз';
 const [form,setForm]=useState({title:first,slug:first.toLowerCase().replaceAll(' ','-'),yandex:'https://music.yandex.ru',vk:'https://vk.com/music',apple:'https://music.apple.com',spotify:''});
 const [copied,setCopied]=useState('');
 const set=(k,v)=>setForm(f=>({...f,[k]:v}));
 const create=()=>onCreateLink(form);
 const copy=async url=>{await navigator.clipboard?.writeText(url);setCopied(url);setTimeout(()=>setCopied(''),2500)};
 return <div className="page fade"><div className="page-title"><div><span className="eyebrow">SMART LINKS</span><h1>Промо-ссылки</h1><p>Создавайте одну ссылку, где слушатель увидит все площадки релиза.</p></div><button className="primary" onClick={create}><Plus/>Создать линкс</button></div><div className="panel link-builder smart-form"><PanelHead title="Новый линкс" action=""/><div className="field-row"><label>Релиз<select value={form.title} onChange={e=>{set('title',e.target.value);set('slug',e.target.value.toLowerCase().replaceAll(' ','-'))}}>{(releases.length?releases:[{id:'empty',title:first}]).map(r=><option key={r.id}>{r.title}</option>)}</select></label><label>Slug<input value={form.slug} onChange={e=>set('slug',e.target.value)}/></label></div><div className="field-row"><label>Яндекс Музыка<input value={form.yandex} onChange={e=>set('yandex',e.target.value)}/></label><label>VK Музыка<input value={form.vk} onChange={e=>set('vk',e.target.value)}/></label></div><div className="field-row"><label>Apple Music<input value={form.apple} onChange={e=>set('apple',e.target.value)}/></label><label>Spotify<input value={form.spotify} onChange={e=>set('spotify',e.target.value)} placeholder="необязательно"/></label></div></div><div className="link-grid">{links.map(item=><article className="link-card" key={item.id}><div><Link/><span>{item.title}</span></div><h3>{item.url}</h3><div>{item.links.map(x=><i key={x}>{x}</i>)}</div><div className="link-actions"><button className="secondary" onClick={()=>copy(item.url)}><Copy/>{copied===item.url?'Скопировано':'Скопировать'}</button><button className="secondary danger-btn" onClick={()=>onDeleteLink(item.id)}><X/>Удалить</button></div></article>)}</div></div>
}

function Support({onSendSupport,currentUser}){
 const [tickets,setTickets]=useState([{id:1,subject:'Вопрос по релизу',status:'Открыт',unread:true,messages:[{me:false,text:'Привет! Команда поддержки на связи. Чем можем помочь?',time:'18:42'}]},{id:2,subject:'Системные уведомления',status:'Авто',unread:false,messages:[{me:false,text:'Здесь будут статусы релизов, выплат и модерации.',time:'авто'}]}]);
 const [active,setActive]=useState(1); const [text,setText]=useState('');
 const ticket=tickets.find(t=>t.id===active)||tickets[0];
 const createTicket=()=>{const item={id:Date.now(),subject:'Новый вопрос',status:'Открыт',unread:false,messages:[{me:false,text:'Новый диалог создан. Опишите вопрос одним сообщением.',time:'сейчас'}]};setTickets(list=>[item,...list]);setActive(item.id)};
 const send=async()=>{if(text.trim()){const value=text;setTickets(list=>list.map(t=>t.id===active?{...t,messages:[...t.messages,{me:true,text:value,time:'сейчас'}]}:t));setText('');await onSendSupport(value)}};
 const closeTicket=()=>setTickets(list=>list.map(t=>t.id===active?{...t,status:'Закрыт'}:t));
 return <div className="page fade support"><div className="support-list"><div className="support-title"><h2>Поддержка</h2><button className="icon" onClick={createTicket}><Plus/></button></div>{tickets.map(t=><button className={`ticket ${t.id===active?'active':''}`} key={t.id} onClick={()=>setActive(t.id)}><span>{t.subject.slice(0,2).toUpperCase()}</span><div><b>{t.subject}</b><p>{t.messages[t.messages.length-1]?.text}</p></div><small>{t.status}{t.unread&&<i/>}</small></button>)}</div><div className="chat"><div className="chat-head"><span>{ticket.subject.slice(0,2).toUpperCase()}</span><div><b>{ticket.subject}</b><small><i/> {currentUser?.role==='artist'?'команда Insomnia':'диалог с артистом'} · {ticket.status}</small></div><button className="secondary close-ticket" onClick={closeTicket}>Закрыть</button></div><div className="messages"><div className="date">Сегодня</div>{ticket.messages.map((m,i)=><div className={'message '+(m.me?'me':'')} key={i}><p>{m.text}</p><small>{m.time}</small></div>)}</div><div className="composer"><button className="icon" title="Вложения будут добавлены следующим этапом"><Plus/></button><input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder={ticket.status==='Закрыт'?'Тикет закрыт':'Напишите сообщение...'}/><button className="send" disabled={ticket.status==='Закрыт'} onClick={send}><Send/></button></div></div></div>
}

function Profile({currentUser,onSaveProfile,onAvatarUpload}){
 const [artist,setArtist]=useState(currentUser.artist||'');const [bio,setBio]=useState(currentUser.bio||'');const fileRef=useRef(null);
 const avatar=currentUser.avatar_url;
 return <div className="page fade upload-page"><div className="page-title"><div><span className="eyebrow">КАБИНЕТ</span><h1>Профиль артиста</h1><p>Аватарка, имя артиста и описание для команды.</p></div><button className="primary" onClick={()=>onSaveProfile({artist_name:artist,bio})}><Check/>Сохранить</button></div><div className="panel profile-panel"><input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={e=>e.target.files?.[0]&&onAvatarUpload(e.target.files[0])}/><button className="profile-avatar" onClick={()=>fileRef.current?.click()}>{avatar?<img src={apiFileUrl(avatar)} alt="avatar"/>:<span>{(artist||'IM').slice(0,2).toUpperCase()}</span>}<small>Загрузить аватарку</small></button><div><label>Имя артиста<input value={artist} onChange={e=>setArtist(e.target.value)}/></label><label>Описание<textarea value={bio} onChange={e=>setBio(e.target.value)} placeholder="Коротко об артисте, жанре, ссылках и достижениях"/></label></div></div></div>
}
...
function AdminTeam({admins,onAddAdmin,onRemoveAdmin}){
 ...
 const set=(k,v)=>setForm(f=>({...f,[k]:v}));
 const submit=()=>{onAddAdmin(form);setForm({email:'',name:'',password:'admin123',role:'admin'});setOpen(false)};
 return <div><div className="page-title compact"><div><span className="eyebrow">КОМАНДА</span><h1>Администраторы</h1><p>Владелец может добавлять и удалять админов.</p></div><button className="primary" onClick={()=>setOpen(!open)}><Plus/>Добавить администратора</button></div>{open&&<div className="panel admin-form"><PanelHead title="Новый администратор" action=""/><div className="field-row"><label>Имя<input value={form.name} onChange={e=>set('name',e.target.value)} placeholder="Алина Исаева"/></label><label>Email<input value={form.email} onChange={e=>set('email',e.target.value)} placeholder="admin@example.com"/></label></div><div className="field-row"><label>Пароль<input value={form.password} onChange={e=>set('password',e.target.value)} placeholder="минимум 6 символов"/></label><label>Роль<select value={form.role} onChange={e=>set('role',e.target.value)}><option value="admin">Админ</option><option value="moderator">Модератор</option><option value="support">Поддержка</option></select></label></div><div className="form-actions"><button className="secondary" onClick={()=>setOpen(false)}>Отмена</button><button className="primary" onClick={submit}><Check/>Создать</button></div></div>}<div className="panel admin-table"><div className="tr th"><span>Администратор</span><span>Роль</span><span>Доступ</span><span>Статус</span></div>{admins.map((x,i)=><div className="tr" key={x.id}><span className="admin-name"><i>{x.initials}</i><b>{x.name}</b></span><span>{x.role}</span><span>{x.access}</span><span className="online">Активен {i>0&&<button onClick={()=>onRemoveAdmin(x.id)}>Удалить</button>}</span></div>)}</div></div>
}
...
function App(){
 ...
 useEffect(()=>{if(!logged)return;const t=token();if(t==='demo')return;api('/notifications',{token:t}).then(rows=>setNotifications(rows.map(x=>({...x,read:!!x.read})))).catch(()=>{});api('/releases',{token:t}).then(rows=>setReleases(rows.map(normalizeRelease))).catch(e=>pushToast('Не удалось загрузить релизы',e.message,'error'));api('/finance',{token:t}).then(fin=>setPayouts((fin.payouts||[]).map(p=>({id:p.id,userId:p.user_id,artist:currentUser.artist,amount:p.amount,card:p.card_number,status:p.status,date:p.created_at})))).catch(()=>{});if(currentUser.role!=='artist'){api('/admin/accounts',{token:t}).then(rows=>setUsers(rows.map(u=>({id:u.id,email:u.email,artist:u.artist_name,role:u.role,status:u.active?'Активен':'Заблокирован',registered:u.created_at,balance:0,paid:0})))).catch(()=>{});api('/admin/payouts',{token:t}).then(rows=>setPayouts(rows.map(p=>({id:p.id,userId:p.user_id,artist:p.artist_name,amount:p.amount,card:p.card_number,status:p.status,date:p.created_at})))).catch(()=>{});api('/admin/users',{token:t}).then(rows=>setAdmins(rows.map(u=>({id:u.id,initials:u.artist_name.split(' ').map(x=>x[0]).join('').slice(0,2),name:u.artist_name,role:u.role,access:'Админка',active:u.active})))).catch(()=>{})}else{api('/smart-links',{token:t}).then(rows=>setPromoLinks(rows.map(x=>({...x,url:`${location.origin}${x.url}`})))).catch(()=>{})}},[logged,currentUser.role]);
 ...
 const createPromoLink=async data=>{try{const title=typeof data==='string'?data:data.title;const raw=typeof data==='string'?{yandex:'https://music.yandex.ru',vk:'https://vk.com/music',apple:'https://music.apple.com'}:data;const links={};[['Яндекс Музыка','yandex'],['VK Музыка','vk'],['Apple Music','apple'],['Spotify','spotify']].forEach(([name,key])=>{if(raw[key])links[name]=raw[key]});if(!Object.keys(links).length)throw new Error('Добавьте хотя бы одну ссылку на площадку');const t=token();let slug=(raw.slug||title).toLowerCase().replaceAll(' ','-');let item={id:Date.now(),title,slug,url:`${location.origin}/p/${slug}`,links:Object.keys(links)};if(t!=='demo'){const saved=await api('/smart-links',{method:'POST',token:t,body:{title,links}});item={...saved,url:`${location.origin}${saved.url}`,links:Object.keys(saved.links)}}setPromoLinks(list=>[item,...list]);pushToast('Промо-ссылка создана',item.url)}catch(e){pushToast('Ошибка линкса',e.message,'error')}};
 const deletePromoLink=id=>{setPromoLinks(list=>list.filter(x=>x.id!==id));pushToast('Промо-ссылка удалена','Линкс скрыт из списка')};
 ...
 const saveProfile=async body=>{try{const t=token();let updated={...currentUser,artist:body.artist_name,bio:body.bio};if(t!=='demo'){const saved=await api('/me',{method:'PATCH',token:t,body});updated={...currentUser,artist:saved.artist_name,bio:saved.bio,avatar_url:saved.avatar_url}}localStorage.setItem('im_user',JSON.stringify(updated));setCurrentUser(updated);pushToast('Профиль сохранён','Данные артиста обновлены')}catch(e){pushToast('Ошибка профиля',e.message,'error')}};
 ...
 const linkFromRelease=release=>{createPromoLink(release.title);setPage('Промо-ссылки')};
 const baseSearch=[
 ...
 let content=currentUser.role!=='artist'&&['Админка','Кабинеты','Финансы','Команда','Релизы'].includes(page)
   ...
   : page==='Главная'?<HomePage setPage={setPage} releases={artistReleases}/>:page==='Релизы'?<Releases releases={artistReleases} setPage={setPage} onOpenRelease={openRelease}/>:page==='Релиз'?<ReleaseDetail release={selectedRelease} setPage={setPage} onPromo={promoFromRelease} onLink={linkFromRelease}/>:page==='Загрузить релиз'?<UploadRelease setPage={setPage} onCreated={createRelease} onUpload={uploadFile} currentUser={currentUser}/>:page==='Статистика'?<Stats releases={artistReleases}/>:page==='Финансы'?<Finance payouts={payouts} onPayoutRequest={requestPayout} currentUser={currentUser}/>:page==='Новости'?<News isStaff={currentUser.role!=='artist'}/>:page==='Промо'?<Promo releases={artistReleases} onPromoRequest={createPromoRequest}/>:page==='Промо-ссылки'?<PromoLinks links={promoLinks} onCreateLink={createPromoLink} onDeleteLink={deletePromoLink} releases={artistReleases}/>:page==='Поддержка'?<Support onSendSupport={sendSupport} currentUser={currentUser}/>:page==='Профиль'?<Profile currentUser={currentUser} onSaveProfile={saveProfile} onAvatarUpload={uploadAvatar}/>:<HomePage setPage={setPage} releases={artistReleases}/>;
