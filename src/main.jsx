...
function News({isStaff=false,items=[],onCreateNews,onDeleteNews}){
 ...
 const add=async()=>{if(form.title&&form.body){await onCreateNews?.(form);setForm({category:'ГАЙД',title:'',body:''})}};
...
function AdminDashboard({releases,users,payouts,admins,promoRequests,news,currentUser,onModerate,onUpdateRelease,onBlockUser,onPayPayout,onAddFinance,onAddAdmin,onRemoveAdmin,onCreateNews,onDeleteNews,section='Админка'}){
 const sectionTab=['Релизы','Кабинеты','Финансы','Промо','Поддержка','Новости','Команда'].includes(section)?section:'Релизы';
 ...
 return <div className="page fade"><div className="page-title"><div><span className="eyebrow">АДМИН-САЙТ</span><h1>Центр управления</h1><p>Все релизы, кабинеты, выплаты, промо, поддержка и команда — в одном месте.</p></div><div className="admin-kpis"><span>{pending} на модерации</span><span>{waiting} выплат</span><span>{users.length} кабинета</span></div></div><div className="tabs admin-tabs">{['Релизы','Кабинеты','Финансы','Промо','Поддержка','Новости','Команда'].map(x=><button className={tab===x?'active':''} onClick={()=>{setTab(x);setSelected(null)}} key={x}>{x}</button>)}</div>{tab==='Релизы'&&selected&&<AdminReleaseDetail release={selected} onBack={()=>setSelected(null)} onModerate={onModerate} onUpdateRelease={onUpdateRelease}/>} {tab==='Релизы'&&!selected&&<AdminReleases releases={releases} onOpen={setSelected} onModerate={onModerate} onUpdateRelease={onUpdateRelease}/>} {tab==='Кабинеты'&&<AdminAccounts users={users} onBlockUser={onBlockUser}/>} {tab==='Финансы'&&<AdminFinance users={users} payouts={payouts} onPayPayout={onPayPayout} onAddFinance={onAddFinance}/>} {tab==='Промо'&&<AdminPromo requests={promoRequests} releases={releases}/>} {tab==='Поддержка'&&<Support currentUser={currentUser}/>} {tab==='Новости'&&<News isStaff items={news} onCreateNews={onCreateNews} onDeleteNews={onDeleteNews}/>} {tab==='Команда'&&<AdminTeam admins={admins} onAddAdmin={onAddAdmin} onRemoveAdmin={onRemoveAdmin}/>}</div>
}
...
function AdminFinance({users,payouts,onPayPayout,onAddFinance}){
...
function App(){
 ...
 const [users,setUsers]=useState(demoUsers);const [payouts,setPayouts]=useState(demoPayouts);const [admins,setAdmins]=useState(demoAdmins);const [promoLinks,setPromoLinks]=useState(demoPromoLinks);const [news,setNews]=useState(demoNews);const [promoRequests,setPromoRequests]=useState([]);
 ...
 const addFinance=async body=>{try{const t=token();if(!body.user_id||!body.amount)throw new Error('Выберите кабинет и сумму');if(t!=='demo')await api('/admin/finance',{method:'POST',token:t,body:{user_id:Number(body.user_id),period:body.period,source:body.source,amount:Number(body.amount),status:'Начислено'}});setUsers(list=>list.map(u=>u.id===Number(body.user_id)?{...u,balance:(u.balance||0)+Number(body.amount)}:u));pushToast('Начисление добавлено',`${Number(body.amount).toLocaleString('ru')} ₽`) }catch(e){pushToast('Ошибка начисления',e.message,'error')}};
 ...
 let content=currentUser.role!=='artist'
   ? <AdminDashboard section={page} releases={releases} users={users} payouts={payouts} admins={admins} promoRequests={promoRequests} news={news} currentUser={currentUser} onModerate={moderateRelease} onUpdateRelease={updateRelease} onBlockUser={blockUser} onPayPayout={payPayout} onAddFinance={addFinance} onAddAdmin={addAdmin} onRemoveAdmin={removeAdmin} onCreateNews={createNews} onDeleteNews={deleteNews}/>
