...
function Topbar({title,setOpen,setPage,theme,onToggleTheme,currentUser}){
  ...
  return <header><button className="icon mobile" onClick={()=>setOpen(true)}><Menu/></button><div><span>INSOMNIA MARKET</span><h2>{title}</h2></div><div className="top-actions"><button className="search"><Search/>Поиск</button><ThemeToggle theme={theme} onToggle={onToggleTheme}/><button className="icon notification"><Bell/><i/></button><button className="avatar" onClick={()=>setPage(currentUser.role==='artist'?'Профиль':'Админка')}>{initials}</button></div></header>
}
...
function App(){
 ...
 const login=async data=>{if(DEMO_MODE){const staff=data.email.includes('owner')||data.email.includes('admin');setSession(staff?{id:10,artist:'Команда Insomnia',role:'owner'}:{id:1,artist:data.artist_name||'Luna Ray',role:'artist'});return}const endpoint=data.register?'register':'login';const body=data.register?{email:data.email,password:data.password,artist_name:data.artist_name}:{email:data.email,password:data.password};const json=await api(`/auth/${endpoint}`,{method:'POST',body});setSession({id:json.user.id,artist:json.user.artist_name,role:json.user.role,avatar_url:json.user.avatar_url,bio:json.user.bio},json.token)};
 ...
 useEffect(()=>{if(!logged)return;const t=token();if(t==='demo')return;api('/releases',{token:t}).then(rows=>setReleases(rows.map(normalizeRelease))).catch(console.error);api('/finance',{token:t}).then(fin=>setPayouts((fin.payouts||[]).map(p=>({id:p.id,userId:p.user_id,artist:currentUser.artist,amount:p.amount,card:p.card_number,status:p.status,date:p.created_at})))).catch(()=>{});if(currentUser.role!=='artist'){api('/admin/accounts',{token:t}).then(rows=>setUsers(rows.map(u=>({id:u.id,email:u.email,artist:u.artist_name,role:u.role,status:u.active?'Активен':'Заблокирован',registered:u.created_at,balance:0,paid:0})))).catch(()=>{});api('/admin/payouts',{token:t}).then(rows=>setPayouts(rows.map(p=>({id:p.id,userId:p.user_id,artist:p.artist_name,amount:p.amount,card:p.card_number,status:p.status,date:p.created_at})))).catch(()=>{});api('/admin/users',{token:t}).then(rows=>setAdmins(rows.map(u=>({id:u.id,initials:u.artist_name.split(' ').map(x=>x[0]).join('').slice(0,2),name:u.artist_name,role:u.role,access:'Админка',active:u.active})))).catch(()=>{})}else{api('/smart-links',{token:t}).then(rows=>setPromoLinks(rows.map(x=>({...x,url:`${location.origin}${x.url}`})))).catch(()=>{})}},[logged,currentUser.role]);
 const uploadFile=async file=>{const t=token();if(t==='demo')return {url:URL.createObjectURL(file)};const data=new FormData();data.append('file',file);return api('/uploads',{method:'POST',body:data,form:true,token:t})};
 const createRelease=async release=>{release={...release,userId:currentUser.id,artist:currentUser.artist,genre:release.genre||'Pop',releaseDate:release.release_date,upc:'',isrc:'',platforms:release.platforms||[],comment:release.comment};const t=token();if(t!=='demo'){const saved=await api('/releases',{method:'POST',token:t,body:{title:release.title,release_type:release.release_type,genre:release.genre,language:release.language,release_date:release.release_date,cover_url:release.cover_url,audio_url:release.audio_url,lyrics:release.lyrics,contributors:JSON.stringify(release.contributors||{}),platforms:release.platforms,comment:release.comment}});release=normalizeRelease(saved)}setReleases(list=>[release,...list])};
 const moderateRelease=async(id,status,reason='')=>{const t=token();if(t!=='demo')await api(`/admin/releases/${id}/moderation`,{method:'PATCH',token:t,body:{status,reason}});setReleases(list=>list.map(r=>r.id===id?{...r,status,reason:status==='Черновик'?reason:'',upc:status==='Принят'&&!r.upc?'463011990'+String(id).padStart(4,'0'):r.upc,isrc:status==='Принят'&&!r.isrc?'RUIIM26'+String(id).padStart(5,'0'):r.isrc}:r))};
 const updateRelease=async(id,patch)=>{const t=token();if(t!=='demo'){if('streams'in patch)await api(`/admin/releases/${id}/stats`,{method:'PATCH',token:t,body:{streams:patch.streams}});if('upc'in patch||'isrc'in patch){const old=releases.find(r=>r.id===id)||{};await api(`/admin/releases/${id}/codes`,{method:'PATCH',token:t,body:{upc:patch.upc??old.upc,isrc:patch.isrc??old.isrc}})}}setReleases(list=>list.map(r=>r.id===id?{...r,...patch}:r))};
 const requestPayout=async card=>{const amount=86420.5;const t=token();let item={id:Date.now(),userId:currentUser.id,artist:currentUser.artist,amount,card,status:'Ожидает выплаты',date:'сегодня'};if(t!=='demo'){const saved=await api('/finance/payouts',{method:'POST',token:t,body:{amount,card_number:card}});item={...item,id:saved.id}}setPayouts(list=>[item,...list])};
 const payPayout=async id=>{const t=token();if(t!=='demo')await api(`/admin/payouts/${id}`,{method:'PATCH',token:t,body:{status:'Оплачено'}});setPayouts(list=>list.map(p=>p.id===id?{...p,status:'Оплачено'}:p))};
 const blockUser=async id=>{const user=users.find(u=>u.id===id);const active=user?.status==='Заблокирован';const t=token();if(t!=='demo')await api(`/admin/accounts/${id}`,{method:'PATCH',token:t,body:{active}});setUsers(list=>list.map(u=>u.id===id?{...u,status:u.status==='Заблокирован'?'Активен':'Заблокирован'}:u))};
 ...
 const createPromoLink=async title=>{const links={'Яндекс Музыка':'https://music.yandex.ru','VK Музыка':'https://vk.com/music','Apple Music':'https://music.apple.com'};const t=token();let item={id:Date.now(),title,slug:title.toLowerCase().replaceAll(' ','-'),url:`${location.origin}/p/${title.toLowerCase().replaceAll(' ','-')}`,links:Object.keys(links)};if(t!=='demo'){const saved=await api('/smart-links',{method:'POST',token:t,body:{title,links}});item={...saved,url:`${location.origin}${saved.url}`,links:Object.keys(saved.links)}}setPromoLinks(list=>[item,...list])};
 const createPromoRequest=async body=>{const t=token();if(t!=='demo')await api('/promo/requests',{method:'POST',token:t,body})};
 const sendSupport=async text=>{const t=token();if(t==='demo')return;let tickets=[];try{tickets=await api('/support/tickets',{token:t})}catch{}let ticket=tickets[0];if(!ticket)ticket=await api('/support/tickets',{method:'POST',token:t,body:{subject:'Вопрос из кабинета'}});await api(`/support/tickets/${ticket.id}/messages`,{method:'POST',token:t,body:{text}})};
 const saveProfile=async body=>{const t=token();let updated={...currentUser,artist:body.artist_name,bio:body.bio};if(t!=='demo'){const saved=await api('/me',{method:'PATCH',token:t,body});updated={...currentUser,artist:saved.artist_name,bio:saved.bio,avatar_url:saved.avatar_url}}localStorage.setItem('im_user',JSON.stringify(updated));setCurrentUser(updated)};
 const uploadAvatar=async file=>{const t=token();let url=URL.createObjectURL(file);if(t!=='demo'){const data=new FormData();data.append('file',file);const saved=await api('/me/avatar',{method:'POST',body:data,form:true,token:t});url=saved.url}const updated={...currentUser,avatar_url:url};localStorage.setItem('im_user',JSON.stringify(updated));setCurrentUser(updated)};
