...
function Support({onSendSupport}){const [messages,setMessages]=useState([{me:false,text:'Привет! Команда поддержки на связи. Чем можем помочь?',time:'18:42'}]);const [text,setText]=useState('');const send=async()=>{if(text.trim()){const value=text;setMessages([...messages,{me:true,text:value,time:'сейчас'}]);setText('');await onSendSupport(value)}};return <div className="page fade support"><div className="support-list"><div className="support-title"><h2>Поддержка</h2><button className="icon" onClick={()=>setMessages([{me:false,text:'Новый диалог создан. Опишите вопрос одним сообщением.',time:'сейчас'}])}><Plus/></button></div><div className="ticket active"><span>A</span><div><b>Команда Insomnia</b><p>{messages[messages.length-1]?.text}</p></div><small>сейчас<i/></small></div><div className="ticket"><span>IM</span><div><b>Системные уведомления</b><p>Статусы релизов и выплат</p></div><small>авто</small></div></div><div className="chat"><div className="chat-head"><span>A</span><div><b>Поддержка</b><small><i/> команда Insomnia · онлайн</small></div><button className="icon" onClick={()=>alert('Поддержка отвечает в рабочее время. Срочные вопросы можно продублировать на почту проекта.')}><CircleHelp/></button></div><div className="messages"><div className="date">Сегодня</div>{messages.map((m,i)=><div className={'message '+(m.me?'me':'')} key={i}><p>{m.text}</p><small>{m.time}</small></div>)}</div><div className="composer"><button className="icon" onClick={()=>alert('Вложения в чат добавим следующим шагом, файлы релиза уже загружаются в форме релиза.')}><Plus/></button><input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder="Напишите сообщение..."/><button className="send" onClick={send}><Send/></button></div></div></div>}
...
function App(){
 ...
 const readAll=()=>setNotifications(list=>list.map(x=>({...x,read:true})));
 ...
 const addAdmin=async()=>{const email=prompt('Почта администратора');if(!email)return;const name=prompt('Имя администратора')||'Новый админ';const password=prompt('Пароль минимум 6 символов')||'admin123';const t=token();let admin={id:Date.now(),initials:name.split(' ').map(x=>x[0]).join('').slice(0,2),name,role:'admin',access:'Админка',active:true};if(t!=='demo'){const saved=await api('/admin/users',{method:'POST',token:t,body:{email,password,name,role:'admin'}});admin={...admin,id:saved.id}}setAdmins(list=>[...list,admin])};
 const removeAdmin=async id=>{const t=token();if(t!=='demo')await api(`/admin/users/${id}`,{method:'DELETE',token:t});setAdmins(list=>list.filter(a=>a.id!==id))};
